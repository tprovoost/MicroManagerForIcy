...
// CVS: $Id: PositionListDlg.java 9800 2012-08-19 19:21:40Z nico $
...
public class PositionListDlg extends MMDialog implements MouseListener
{
    ...
    private static FileType POSITION_LIST_FILE = new FileType("POSITION_LIST_FILE", "Position list file",
            System.getProperty("user.home") + "/PositionList.pos", true, "pos");
    ...
    /**
     * List with Axis data. Currently, we use only a single global instance of
     * this class
     */
     ...
     * Create the dialog
     */
    public PositionListDlg(CMMCore core, ScriptInterface gui, PositionList posList, MMOptions opts, AcqControlDlg dlg)
    {
        super();
        ...
        springLayout.putConstraint(SpringLayout.NORTH, axisPane, -(23 + 23 * axisModel.getRowCount()),
        ...
        springLayout.putConstraint(SpringLayout.NORTH, markButton, northConstraint, SpringLayout.NORTH,
                getContentPane());
        springLayout.putConstraint(SpringLayout.SOUTH, markButton, northConstraint += buttonHeight, SpringLayout.NORTH,
                getContentPane());
        ...
        posTable_.addFocusListener(new java.awt.event.FocusAdapter()
        {
        ...
        springLayout.putConstraint(SpringLayout.SOUTH, upButton, northConstraint + buttonHeight, SpringLayout.NORTH,
                getContentPane());
        ...
        downButton
                .setToolTipText("Move currently selected position down list (lower positions on list are acquired later)");
        getContentPane().add(downButton);
        springLayout.putConstraint(SpringLayout.NORTH, downButton, northConstraint, SpringLayout.NORTH,
                getContentPane());
        // increment northConstraint for next button....
        springLayout.putConstraint(SpringLayout.SOUTH, downButton, northConstraint += buttonHeight, SpringLayout.NORTH,
                getContentPane());
        ...
        springLayout.putConstraint(SpringLayout.NORTH, gotoButton, northConstraint, SpringLayout.NORTH,
                getContentPane());
        springLayout.putConstraint(SpringLayout.SOUTH, gotoButton, northConstraint += buttonHeight, SpringLayout.NORTH,
                getContentPane());
        ...
        springLayout.putConstraint(SpringLayout.NORTH, refreshButton, northConstraint, SpringLayout.NORTH,
                getContentPane());
        springLayout.putConstraint(SpringLayout.SOUTH, refreshButton, northConstraint += buttonHeight,
        ...
        springLayout.putConstraint(SpringLayout.NORTH, removeButton, northConstraint, SpringLayout.NORTH,
                getContentPane());
        springLayout.putConstraint(SpringLayout.SOUTH, removeButton, northConstraint += buttonHeight,
                SpringLayout.NORTH, getContentPane());
        ...
        setOriginButton
                .setToolTipText("Drives X and Y stages back to their original positions and zeros their position values");
        getContentPane().add(setOriginButton);
        springLayout.putConstraint(SpringLayout.NORTH, setOriginButton, northConstraint, SpringLayout.NORTH,
                getContentPane());
        springLayout.putConstraint(SpringLayout.SOUTH, setOriginButton, northConstraint += buttonHeight,
                SpringLayout.NORTH, getContentPane());
        ...
        springLayout.putConstraint(SpringLayout.NORTH, removeAllButton, northConstraint, SpringLayout.NORTH,
                getContentPane());
        springLayout.putConstraint(SpringLayout.SOUTH, removeAllButton, northConstraint += buttonHeight,
                SpringLayout.NORTH, getContentPane());
    ...
    protected void clearAllPositions()
    {
        ...
        ptm.fireTableDataChanged();
        ((MMMainFrame) gui_).getAcqDlgPatched().updateGUIContents();
    }
    ...
    /**
     * Store current xyPosition. Use data collected in refreshCurrentPosition()
     */
    public void markPosition()
    {
        ...
        if (selMsp == null)
        {
            ...
            ptm.fireTableDataChanged();
            ((MMMainFrame) gui_).getAcqDlgPatched().updateGUIContents();
            // gui_.getAcqDlgPatched().updateGUIContents();
        }
        else
    ...
    private void calibrate()
    {
        ...
        if (JOptionPane.YES_OPTION != JOptionPane.showOptionDialog(this, "Really calibrate your XY stage?",
                "Are you sure?", JOptionPane.DEFAULT_OPTION, JOptionPane.WARNING_MESSAGE, null, options, options[1]))
            return;
            ...
            try
            {
                ...
                {// stop the calibration
                    ...
                    option = JOptionPane.showOptionDialog(d, "RESUME calibration?", "Calibration",
                            JOptionPane.OK_OPTION, JOptionPane.QUESTION_MESSAGE, null, options2, options2[0]);

                    if (option == 1)
                        return; // not to resume, just stop
    ...
    /*
     * Implementation of MouseListener Sole purpose is to be able to unselect
     * rows in the positionlist table
     */
    ...
    /*
     * This event is fired after the table sets its selection Remember where was
     * clicked previously so as to allow for toggling the selected row
     */
