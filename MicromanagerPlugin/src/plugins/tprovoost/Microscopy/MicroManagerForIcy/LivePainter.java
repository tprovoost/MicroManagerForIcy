...
public class LivePainter extends MicroscopePainter {
	...
	if (e.isAltDown()) {
		...
		lastPosStage = new Point2D.Double(xyz[0], xyz[1]);
	...
	if (e.isAltDown()) {
		...
		if (lastImagePoint != null) {
			...
			pxsize = 1;
		    try {
			StageMover.moveXYAbsolute(lastPosStage.x + vect.getX() * pxsize, lastPosStage.y + vect.getY() * pxsize);
			e.consume();
		    } catch (Exception e1) {
			e1.printStackTrace();
		    }
